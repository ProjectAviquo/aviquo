...
class Tag(models.Model):
    ...
    name = models.CharField(
        ...
        max_length=50,
        verbose_name="Tag",
        help_text="Example: scholarship, extracurricular activity, etc.",
    )
...
class Opportunity(models.Model):
    ...
    def __str__(self):
        return (
            self.name
        )  # Change to return the name for a more descriptive representation
    ...
    bookmarks = models.ManyToManyField(
        settings.AUTH_USER_MODEL, blank=True, related_name="bookmarked_opportunities"
    )
    bookmarked_users = models.ManyToManyField(
        settings.AUTH_USER_MODEL,
        blank=True,
        related_name="bookmarked_opportunities_set",
    )
